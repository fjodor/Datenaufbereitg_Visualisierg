...
### Mehrere Infos in einer Zelle, mit bedeutungsvollen Formatierungen

![Mehrere Infos in einer Zelle](in-cell-formatting.png)

Basiert auf realem Beispiel!
...
* Produkt-ID
* Test vs. Produktion
* Formatierung: rot, durchgestrichen bedeutet Fehler

Generelle Lösung: *unnesting* wie oben; einfacher: map über genisteten Datensatz; eine Info herausziehen
...
Diese Info dann zu den restlichen Daten hinzuspielen:
...
![Hochgestellte Symbole](superscript-symbols.png)

Gefährliche Situation: Welche Punktzahl hat Paula erreicht?
...
Richtige Antwort: Es war nicht Paula, sondern Paul.

Spalte "Score" wurde als *character* formatiert, sodass die hochgestellte 1 eingetragen werden konnte; Excel erlaubt keine hochgestellten Symbole in numerischen Zellen! Man könnte das leicht für einen Fehler halten und mit as.integer() zurück-transformieren.

Mit tidyxl jedoch kann man die Zeilen jedes Elements der *character_formatted*-Spalte zählen und so Zellen mit Formatierungen identifizieren.
...
Dann kann man die entsprechenden Werte und Symbole separat behandeln. Annahme: Wert = erster Eintrag; Symbol = zweiter Eintrag.
...
## Nicht-Texte in Spaltenüberschriften, z. B. Datumsformat**

readxl konvertiert aktuell nicht zu R-Datumsformat

Mit tidyxl und unpivotr kann man solche Zellen vor *spatter()* formatieren.  
Fortgeschritten: im *behead()*-Aufruf Argument *formatters* angeben, z. B.:
...
Abrufbar in der *comment*-Spalte, so wie *character*. Kommentare können formatiert sein, aber das zu berücksichtigen ist (noch?) nicht in tidyxl implementiert.
...
Aktuell noch nicht ausgearbeitetes "Gotcha": Benannte Zellbereiche (*named ranges*).
...
* Excel-Mappen liegen im **smungs**-Paket auf GitHub.
* Es gibt 7 Fallstudien sowie weitere Beispiele auf Youtube etc.
